...
LOOP:
  ...
  > 分支检查（仅当 stage-status.json 中本 feature 有 repos 数组时执行）：
  > 在开始编码前，对 repos 数组中每个 branchCreated == true 的仓库：
  >   1. `cd {repo-path} && git branch --show-current`
  >   2. 如果当前分支不是 featureBranch → `git checkout {featureBranch}`
  >   3. 切换失败 → 记录 WARNING 到 errors 数组，继续（不阻塞编码）
  ...
  if restart_type == BUG_FIX:
    ...
    3. 逐里程碑实现（仅执行状态为 pending 的里程碑，done 的跳过）：
       ...
       c. 通过 → 标记里程碑 `[x]` 并在 Progress Log 追加完成记录（日期+里程碑+done+备注）
       c2. （推荐）Git commit（在特性分支上，如已配置 repos）：
           `cd {repo-path} && git add -A && git commit -m "M{NNN}: {description}"`
           如有多个仓库涉及修改，在每个仓库中分别 commit
  ...
  ── Tier 1: 单元测试 ──
  3. 使用 Bash 执行单元测试命令（来自 CLAUDE.md §8 或 test-plan.md）
  4. P0 FAIL → 进入 BUG_FIX 流程

  ── Tier 2: 集成/API 测试 ──
  5. 使用 Bash 执行集成测试命令
  6. P0 FAIL → 进入 BUG_FIX 流程

  ── Tier 3: E2E 测试（条件执行）──
  7. 检查 test-cases.md 中是否存在 E2E 用例
     - 无 E2E 用例 → 跳过 Tier 3，直接进入覆盖矩阵汇总
     - 有 E2E 用例 → 继续
  8. 服务启动（按 qa-agent.md E2E 节的服务生命周期管理执行）：
     a. Bash run_in_background 启动后端
     b. 健康检查轮询（超时 → 所有 E2E 用例标记 BLOCKED）
     c. Bash run_in_background 启动前端
     d. browser_navigate + browser_snapshot 验证前端可达
  9. 逐条执行 E2E 用例（P0 不可跳过）：
     - Prong A（步骤 <= 5）→ MCP 直接执行
     - Prong B（步骤 > 5）→ 生成 .spec.ts 后 npx playwright test
  10. 服务关停 TEARDOWN（无条件执行）：
      a. 停止后端和前端后台进程
      b. browser_close 关闭浏览器
  11. 输出覆盖矩阵（含三层用例执行状态）
  12. 更新 stage-status.json: subStage=testing

  // 测试覆盖完整性门禁
  COVERAGE_CHECK:
    - 覆盖矩阵中是否有未执行的 P0 用例（含 unit/integration/e2e）？
      → 有 → 必须执行，不可跳过
    - 是否所有 P0 用例均为 PASS？
      → 否（有 P0 FAIL）→ 执行 BUG_FIX 流程
      → 否（有 P0 BLOCKED）→ 记录阻塞原因，视情况继续或请求人工介入
    - P1/P2 FAIL 用例 → 记录到 review-log.md，继续
  ...
  > R3 要求文档代码同步。review-log 中发现的规格-代码不一致必须在此处修复。

  1. 读取 docs/specs/{feature}/review-log.md，筛选所有描述"规格与代码不一致"的条目
     （关键词：表名不一致、端点不一致、异常类型不一致、接口签名不一致、SPEC_DRIFT）
  2. 对每个此类条目：
     - 如果代码行为是正确的/已审批的 → 更新规格文档（tech.md / test-cases.md / test-plan.md）使其与代码一致
     - 在 review-log.md 中将该条目标记为 RESOLVED，备注"spec updated to match implementation"
  3. 验证：review-log.md 中无未处理的 SPEC_DRIFT 类条目
...
- ✅ 测试覆盖矩阵完整，P0 用例全部 PASS（含单元/集成/E2E 三层）
- ✅ E2E 测试脚本已生成并提交（如有 Prong B 用例）
...
| 测试覆盖完整且 P0 全通过 | 贴出覆盖矩阵 + Tier 1/2 终端输出 + Tier 3 快照/脚本输出 |
| E2E 脚本已生成 | 列出 testing/e2e/{feature}/ 目录下 .spec.ts 文件路径（如有 Prong B） |
